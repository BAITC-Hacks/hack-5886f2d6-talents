...
сформированные ядром. Для старых результатов отсутствие поля допустимо. Если поле
есть, оно содержит 1–3 уникальные непустые строки, каждая до 200 Unicode-символов
(строка из пробелов не считается действием). Ядро 1.2.0 выдаёт поле всем узлам.
Python сохраняет текст и порядок без изменений. Версия схемы остаётся `1.0`, старые результаты совместимы.

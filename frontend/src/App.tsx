...
export default function App() {
  ...
  function selectClient(gid: string) {
    setSelected(gid); setScope('neighbors'); setHops(1); setRole('all'); setCluster('all'); setSearchMessage('');
  }
  ...
  return <div className="app">
    ...
    <main>
      ...
      <section className="toolbar" aria-label="Поиск и фильтры">
        ...
        <div className="filter-controls"><Filter size={16}/><label><span className="sr-only">Фильтр по роли</span><select aria-label="Фильтр по роли" value={role} onChange={e => { setRole(e.target.value as Role | 'all'); setLimit(50); }}><option value="all">Все роли</option>{roles.map(r => <option key={r} value={r}>{roleNames[r]}</option>)}</select></label>
        <label><span className="sr-only">Фильтр по кластеру</span><select aria-label="Фильтр по кластеру" value={cluster} onChange={e => { setCluster(e.target.value); setScope(e.target.value === 'all' ? 'all' : 'cluster'); setLimit(50); }}><option value="all">Все кластеры</option>{data.clusters.map(c => <option key={c.id} value={c.id}>Кластер {c.id} · {c.count}</option>)}</select></label><button className="icon-button" aria-label="Сбросить фильтры" onClick={reset}><RefreshCw size={16}/></button></div>
      </section>
      ...
      <div className="workspace">
        ...
        <section className="graph-panel" aria-label="Граф переводов">
          ...
          <div className="graph-toolbar"><div className="view-tabs"><button className={scope === 'cluster' ? 'active' : ''} onClick={() => setScope('cluster')}>Кластер</button><button disabled={!client} className={scope === 'neighbors' ? 'active' : ''} onClick={() => setScope('neighbors')}>Окружение</button><button className={scope === 'all' ? 'active' : ''} onClick={() => setScope('all')}>Общий граф</button></div><label className="color-select"><SlidersHorizontal size={14}/><select aria-label="Раскраска графа" value={colorBy} onChange={e => setColorBy(e.target.value as 'role' | 'cluster')}><option value="role">По роли</option><option value="cluster">По кластеру</option></select></label></div>
          {scope === 'neighbors' && <div className="scope-description">Связи в обоих направлениях <button onClick={() => setHops(n => n === 1 ? 2 : 1)}>{hops === 1 ? 'Раскрыть 2 шага' : 'Оставить 1 шаг'}</button><span>Сейчас: {hops}</span></div>}
          ...
          <GraphView nodes={visibleNodes} edges={visibleEdges} selected={selected} colorBy={colorBy} onSelect={selectClient} overview={scope === 'all'}/>
        ...
        {client ? <ClientDetails key={client.gid} client={client}/> : <aside className="details-panel empty-details"><Network size={32}/><h2>Выберите клиента</h2><p>Нажмите на узел, строку списка или найдите полный gid.</p></aside>}
      </div>

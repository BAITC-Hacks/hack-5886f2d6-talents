...
export default function ClientDetails({ client }: { client: Client }) {
  ...
  return <aside className="details-panel" aria-label="Карточка клиента">
    ...
    <section className="next-actions"><h3>Что проверить дальше</h3>
      {client.next_actions === undefined ? <p className="muted">В этом расчёте следующие действия не переданы.</p>
        : client.next_actions.length ? <ol>{client.next_actions.map((action, index) => <li key={index}>{action}</li>)}</ol>
        : <p className="muted">В этом расчёте дополнительные действия не указаны.</p>}
    </section>

...
import { plainLanguage, roleExplanation, priorityExplanation } from './explanations';
...
const breakdownNames: Record<string, string> = { seed_reach: 'Связи с исходными клиентами', volume: 'Объём переводов', counterparties: 'Контрагенты', external_clusters: 'Связи с другими группами' };
...
export default function ClientDetails({ client }: { client: Client }) {
  ...
  return <aside className="details-panel" aria-label="Карточка клиента">
    ...
    <section className="explanation"><h3>Почему эта роль</h3><p>{roleExplanation(client)}</p></section>
    <section className="priority-explanation"><h3>Почему проверять</h3><p>{priorityExplanation(client)}</p></section>
    {!!client.next_actions?.length && <section className="next-actions"><h3>Что проверить дальше</h3>
      <ol>{client.next_actions.map((action, index) => <li key={index}>{plainLanguage(action)}</li>)}</ol>
    </section>}
    <details className="source-explanations"><summary>Исходные формулировки расчёта</summary><p>{client.evidence}</p><p>{client.why}</p>{!!client.next_actions?.length && <ol>{client.next_actions.map((action, i) => <li key={i}>{action}</li>)}</ol>}</details>
    ...
    <p className="muted terms-note">Исходные клиенты — те, с кого началась выгрузка. Кластеры — группы связанных переводами клиентов.</p>
    <section className="metrics-section"><h3>Переводы в выгрузке</h3>
      ...
      <p className="muted">Суммы из выгрузки, включая переводы себе. Не баланс счёта.</p>
      <dl className="facts">
        ...
        <div><dt>Исходных клиентов с цепочкой к этому клиенту</dt><dd>{value(client.seed_reach)}</dd></div>
        <div><dt>Кратчайшая цепочка от другого исходного клиента: шагов</dt><dd>{value(client.min_seed_hops)}</dd></div>
        <div><dt>Шаг выгрузки · depth</dt><dd>{value(client.depth)}</dd></div>

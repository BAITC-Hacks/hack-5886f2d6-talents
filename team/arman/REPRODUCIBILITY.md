...
**Текущий статус:** в `fbec33f` принят [завершённый отчёт Армана](FINAL_RUN.md).
На базе `9e5eff6` созданы новые venv, Release-сборка и node_modules; затем
tracked-файлы обновлены до `859b33f`, а после объединения UI проверен `2e2f385`.
Затронутые проверки повторены: 110 Python-тестов, 34 теста C++ и 22 теста UI
прошли. Зависимости не устанавливались заново на каждом обновлении; это новая
копия на ноутбуке Армана с существующими runtime/toolchain и возможными кэшами.
Ниже сохранены результаты старого `c7f7bb4` и команды для повторения на другом
устройстве. Команды UI также есть в корневом README.

## Историческая проверка c7f7bb4: что именно подтверждено
...
Интерфейс реализован и использует `/data/graph.json`, `/data/nodes_roles.csv`,
`/data/clusters.csv`, `/data/top_nodes.csv` и manifest для проверки комплекта.
После успешного пересчёта запустить из корня репозитория (нужны Node.js 22.14+
и npm):
...
Открыть http://localhost:5173, проверить поиск, карточки, направления и экспорт.
Подробный порядок и ограничения Windows — в [frontend/README.md](../../frontend/README.md).

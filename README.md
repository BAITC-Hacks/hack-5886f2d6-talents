...
Часть Армана по финальному плану: [чистый запуск с UI](team/arman/FINAL_RUN.md)
и [передача `next_actions`](team/arman/NEXT_ACTIONS.md). Python поддерживает
следующие действия из C++ 1.2.0 без изменения CSV. Пакет пересчитан, действия
сохранены для всех 2248 клиентов и отображаются в карточке нового UI Савелия.

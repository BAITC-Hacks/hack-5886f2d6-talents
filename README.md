...
**Версии:** C++ 1.3.0 содержит расчёт устойчивости. Экран Савелия подключён
по [контракту](team/RESILIENCE_CONTRACT.md); готовый пакет UI пока 1.2.0
с действиями для всех клиентов и ожидает пересчёта Арманом. Полный прогон 1.3.0
после объединения с проверками Армана прошёл за 4,316 с: 43 теста C++,
333 Python без пропусков; прежние роли, оценки и три CSV сохранены.
...
После подключения нового экрана прошли 34 теста UI и production-сборка.
Приёмка всех сценариев на новой поставке ожидает пакет 1.3.0.
